...
test.describe("student UI simplification regressions", () => {
    ...
    test("320px dashboard wraps long identity copy, hides zero completion KPI, and stacks assignment CTA", async ({ page }) => {
        ...
        const desktopGrid = await zeroGrid.evaluate(element => {
            const grid = element.getBoundingClientRect();
            const primary = element.querySelector<HTMLElement>(".student-dashboard-primary-task")!.getBoundingClientRect();
            const average = element.querySelector<HTMLElement>(".student-dashboard-average-card")!.getBoundingClientRect();
            const completed = element.querySelector<HTMLElement>(".student-dashboard-completed-task")!.getBoundingClientRect();
            return {
                gridLeft: grid.left,
                ...
                averageLeft: average.left,
                averageRight: average.right,
                averageTop: average.top,
                averageBottom: average.bottom,
                completedLeft: completed.left,
                completedRight: completed.right,
                completedTop: completed.top,
            };
        });
        expect(Math.abs(desktopGrid.primaryLeft - desktopGrid.gridLeft)).toBeLessThanOrEqual(2);
        expect(Math.abs(desktopGrid.averageRight - desktopGrid.gridRight)).toBeLessThanOrEqual(2);
        expect(Math.abs(desktopGrid.completedLeft - desktopGrid.gridLeft)).toBeLessThanOrEqual(2);
        expect(Math.abs(desktopGrid.primaryTop - desktopGrid.averageTop)).toBeLessThanOrEqual(2);
        expect(desktopGrid.averageLeft).toBeGreaterThanOrEqual(desktopGrid.primaryRight - 1);
        expect(desktopGrid.completedRight).toBeLessThanOrEqual(desktopGrid.averageLeft + 1);
        expect(desktopGrid.completedTop).toBeGreaterThanOrEqual(desktopGrid.averageBottom - 1);
        await expectInsideViewport(page, ".student-dashboard-grid > *");
    });
