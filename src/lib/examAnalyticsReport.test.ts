...
describe("exam analytics report headline", () => {
    it.each([
        {
            ...
            input: { hasGradableEvidence: false, lowStudentCount: 0, riskyQuestionCount: 0 },
            detail: "전체 제출 8건 중 채점 가능한 5명 기준 · 미채점 문항은 취약 개념과 행동 추천에서 제외했습니다.",
        },
        {
            name: "missing-rate observation",
            input: { weakConcept: "시제", lowStudentCount: 0, riskyQuestionCount: 0 },
            detail: "전체 제출 8건 중 채점 가능한 5명 기준 · ‘시제’ 정답률을 계산할 수 없습니다.",
        },
        {
            name: "weak-concept action",
            input: { weakConcept: "시제", weakConceptRate: 60, lowStudentCount: 1, riskyQuestionCount: 2 },
            detail: "정답률 60% · 지원 학생 1명 · 점검 문항 2개 · 전체 제출 8건 중 채점 가능한 5명 기준",
        },
        {
            name: "risky-question observation",
            input: { weakConcept: "시제", weakConceptRate: 80, lowStudentCount: 0, riskyQuestionCount: 2 },
            detail: "재점검이 필요한 문항 2개 · 전체 제출 8건 중 채점 가능한 5명 기준입니다.",
        },
        {
            name: "low-student observation",
            input: { weakConcept: "시제", weakConceptRate: 80, lowStudentCount: 1, riskyQuestionCount: 0 },
            detail: "60점 미만 학생 1명 · 전체 제출 8건 중 채점 가능한 5명 기준입니다.",
        },
        {
            name: "positive",
            input: { weakConcept: "시제", weakConceptRate: 80, lowStudentCount: 0, riskyQuestionCount: 0 },
            detail: "전체 제출 8건 중 채점 가능한 5명 기준입니다.",
        },
    ])("uses the mixed evidence basis in the $name branch", ({ input, detail }) => {
        expect(buildExamHeadlineInsight({
            performanceCount: 5,
            totalSubmissionCount: 8,
            ...input,
        })).toMatchObject({ detail });
    });

    it("keeps equal submission and performance counts concise", () => {
        expect(buildExamHeadlineInsight({
            performanceCount: 5,
    ...
    it("does not fabricate an action when every question lacks gradable evidence", () => {
        expect(buildExamHeadlineInsight({
            ...
            detail: "8명 제출 기준 · 미채점 문항은 취약 개념과 행동 추천에서 제외했습니다.",
        });
    });
    ...
    it("prioritizes a weak-concept action over risky questions and low students from the five-submission boundary", () => {
        expect(buildExamHeadlineInsight({
            ...
            detail: "정답률 69% · 지원 학생 2명 · 점검 문항 3개 · 5명 제출 기준",
        });
    });
    ...
    it("prioritizes risky questions over low students when no weak-concept action qualifies", () => {
        expect(buildExamHeadlineInsight({
            ...
            detail: "재점검이 필요한 문항 2개 · 8명 제출 기준입니다.",
        });
    });
    ...
    it("reports students needing support before the positive fallback", () => {
        expect(buildExamHeadlineInsight({
            ...
            detail: "60점 미만 학생 5명 · 5명 제출 기준입니다.",
        });
    });
    ...
    it("sanitizes invalid, negative, and decimal headline metrics", () => {
        expect(buildExamHeadlineInsight({
            ...
            tone: "action",
            title: "‘시제’ 보강이 가장 효과적입니다",
            detail: "정답률 12% · 지원 학생 0명 · 점검 문항 2개 · 5명 제출 기준",
        });
    ...
    it("does not trigger or render a weak-concept action when its rate is missing", () => {
        ...
        expect(insight).toEqual({
            ...
            detail: "9명 제출 기준 · ‘시제’ 정답률을 계산할 수 없습니다.",
        });
    ...
    it("rounds a valid rate to one decimal before rendering and comparing the action threshold", () => {
        expect(buildExamHeadlineInsight({
            ...
            tone: "action",
            title: "‘시제’ 보강이 가장 효과적입니다",
            detail: "정답률 69.9% · 지원 학생 2명 · 점검 문항 1개 · 9명 제출 기준",
        });
        ...
        expect(buildExamHeadlineInsight({
            ...
            tone: "observation",
            title: "문항별 정답률 차이를 먼저 점검해 주세요",
            detail: "재점검이 필요한 문항 1개 · 9명 제출 기준입니다.",
        });
    });
