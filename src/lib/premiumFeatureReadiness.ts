...
export const PREMIUM_FEATURE_READINESS: Record<PlanEntitlementKey, PremiumFeatureReadiness> = {
    ...
    advancedAnalytics: {
        status: "available",
        label: "오답·회복 분석",
        description: "문항 태그와 누적 제출이 있는 범위에서 오답 유형, 반복 약점, 재시험 회복을 분석합니다.",
    },
    ...
    retakeAssignments: {
        status: "available",
        label: "오답 회복 과제",
        description: "원시험의 오답·미응답을 그대로 또는 유형별로 묶어 재응시 링크를 만듭니다. 새 유사문항 생성은 준비 중입니다.",
    },
    multiTeacher: {
        status: "planned",
...
export const BILLING_PLAN_FEATURES: Record<PlanKey, readonly BillingPlanFeature[]> = {
    ...
    pro: [
        ...
        { label: "오답 유형·재시험 회복 분석", status: "available", detail: "문항 태그·누적 제출 범위" },
        { label: "새 유사문항 추천", status: "planned", detail: "권리 확인 문항은행 필요" },
