...
- [ ] **학생별 실수 원인 분석 후속 단계 (프리미엄)**: 문항 개념·함정 분석과 정오답 기반 강약점 이후, 필기/풀이 단계를 판독해 실제 조건 누락·계산 실수·개념 혼동을 구분한다. 교사 검토, 근거 위치, 불확실성 표시를 포함한다. 현재 범위에서는 학생의 실제 실수 원인을 자동 확정하지 않는다.
