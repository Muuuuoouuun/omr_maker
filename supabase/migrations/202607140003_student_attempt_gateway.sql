...
begin
    ...
    if jsonb_typeof(p_question_results) <> 'array' then
    ...
    if exists (
        select 1
         ...
         where results.attempt_id <> v_attempt.id
            or results.exam_id <> v_attempt.exam_id
            or results.organization_id <> v_attempt.organization_id
    ) then
