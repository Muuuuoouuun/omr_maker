...
describe("teacher exam detail loading surface", () => {
    ...
    it("does not present failed canonical attempt reads as zero submissions", () => {
        expect(pageSource).toContain("loadTeacherAttempts(id)");
        expect(pageSource).not.toContain("loadTeacherAttempts(),");
        expect(pageSource).not.toContain("loadedAttempts.items.filter(attempt => attempt.examId === id)");
