...
begin
    ...
    if jsonb_typeof(p_feedback) <> 'object' then
...
create or replace function public.omr_return_feedback_v1(
    p_organization_id text,
    p_feedback_id text,
...
begin
    update public.omr_attempt_feedback
       set status = 'returned',
           ...
           notification_channel = 'in_app',
           notified_at = coalesce(notified_at, p_returned_at),
           returned_at = coalesce(returned_at, p_returned_at),
           updated_at = p_returned_at,
           payload = coalesce(payload, '{}'::jsonb) || jsonb_build_object(
               'status', 'returned',
               'returnedAt', coalesce(returned_at, p_returned_at),
               'updatedAt', p_returned_at,
               'delivery', coalesce(payload->'delivery', '{}'::jsonb) || jsonb_build_object(
                   'notificationStatus', 'queued',
                   'notificationChannel', 'in_app',
                   'notifiedAt', coalesce(notified_at, p_returned_at)
               )
           )
...
create or replace function public.omr_mark_feedback_opened_v2(
    p_organization_id text,
...
begin
    update public.omr_attempt_feedback
       set notification_status = 'sent',
           first_opened_at = coalesce(first_opened_at, p_opened_at),
           last_opened_at = p_opened_at,
           open_count = open_count + 1,
           updated_at = p_opened_at,
           payload = jsonb_set(
               jsonb_set(coalesce(payload, '{}'::jsonb), '{updatedAt}', to_jsonb(p_opened_at), true),
               '{delivery}',
               coalesce(payload->'delivery', '{}'::jsonb) || jsonb_build_object(
                   'notificationStatus', 'sent',
                   'firstOpenedAt', coalesce(first_opened_at, p_opened_at),
                   'lastOpenedAt', p_opened_at,
     ...
     where id = trim(p_feedback_id)
       and organization_id = trim(p_organization_id)
       and student_profile_id = trim(p_student_profile_id)
       and status = 'returned'
    returning * into v_stored;
...
revoke all on function public.omr_mark_feedback_opened_v2(text, text, text, timestamptz) from public, anon, authenticated;
...
grant execute on function public.omr_mark_feedback_opened_v2(text, text, text, timestamptz) to service_role;
