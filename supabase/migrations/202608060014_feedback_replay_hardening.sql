-- Re-deploy feedback replay hardening for databases that already applied 012.
-- Exact mutation receipts are checked before the current plan entitlement, while
-- new mutations remain fail-closed inside the authoritative v2 database RPCs.
...
create or replace function public.omr_save_feedback_v2(
...
create or replace function public.omr_return_feedback_v2(
    p_organization_id text,
    p_feedback_id text,
    p_expected_revision bigint,
    p_mutation_id text
...
comment on function public.omr_save_feedback_v2(text,jsonb,bigint,text)
    is 'feedback-replay-hardening:202608060014';
comment on function public.omr_return_feedback_v2(text,text,bigint,text)
    is 'feedback-replay-hardening:202608060014';
