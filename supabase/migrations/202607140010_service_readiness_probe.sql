...
begin
    ...
    select coalesce(relforcerowsecurity, false)
     ...
     where oid = 'public.omr_exams'::pg_catalog.regclass;

    select coalesce(relforcerowsecurity, false)
      into v_attempts_force_rls
      from pg_catalog.pg_class
     where oid = 'public.omr_attempts'::pg_catalog.regclass;

    select coalesce(relforcerowsecurity, false)
      into v_question_results_force_rls
      from pg_catalog.pg_class
     where oid = 'public.omr_question_results'::pg_catalog.regclass;

    select coalesce(relforcerowsecurity, false)
      into v_student_credentials_force_rls
      from pg_catalog.pg_class
     where oid = 'public.omr_student_start_credentials'::pg_catalog.regclass;

    select coalesce(relforcerowsecurity, false)
      into v_remote_assets_force_rls
      from pg_catalog.pg_class
     where oid = 'public.omr_remote_assets'::pg_catalog.regclass;
