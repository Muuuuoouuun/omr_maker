...
function evidenceBasisLabel(performanceCount: number, totalSubmissionCount: number): string {
    return totalSubmissionCount === performanceCount
        ? `${performanceCount}명 제출 기준`
        : `전체 제출 ${totalSubmissionCount}건 중 채점 가능한 ${performanceCount}명 기준`;
}
...
export function buildExamHeadlineInsight(input: ExamHeadlineInsightInput): ExamHeadlineInsight {
    ...
    const evidenceBasis = evidenceBasisLabel(performanceCount, totalSubmissionCount);

    if (input.hasGradableEvidence === false) {
        return {
            tone: "observation",
            title: "채점 가능한 문항 근거가 더 필요합니다",
            detail: `${evidenceBasis} · 미채점 문항은 취약 개념과 행동 추천에서 제외했습니다.`,
        };
    }
    ...
    if (weakConcept && weakConceptRate === undefined) {
        return {
            ...
            detail: `${evidenceBasis} · ‘${weakConcept}’ 정답률을 계산할 수 없습니다.`,
        };
    }
    ...
    if (weakConcept && weakConceptRate !== undefined && weakConceptRate < 70) {
        return {
            ...
            detail: `정답률 ${weakConceptRate}% · 지원 학생 ${lowStudentCount}명 · 점검 문항 ${riskyQuestionCount}개 · ${evidenceBasis}`,
        };
    }
    ...
    if (riskyQuestionCount > 0) {
        return {
            ...
            detail: `재점검이 필요한 문항 ${riskyQuestionCount}개 · ${evidenceBasis}입니다.`,
        };
    }
    ...
    if (lowStudentCount > 0) {
        return {
            ...
            detail: `60점 미만 학생 ${lowStudentCount}명 · ${evidenceBasis}입니다.`,
        };
    }
