...
describe("service UI surface", () => {
    ...
    it("keeps installed phone and tablet app shells inside safe areas", () => {
        ...
        expect(viewportHeightSync).toContain('window.addEventListener("resize", scheduleApplyHeight, { passive: true })');
        expect(viewportHeightSync).toContain('visualViewport?.addEventListener("resize", scheduleApplyHeight, { passive: true })');
        expect(viewportHeightSync).toContain('visualViewport?.addEventListener("scroll", scheduleApplyHeight, { passive: true })');
        expect(viewportHeightSync).toContain('window.addEventListener("orientationchange", scheduleApplyHeight)');
        expect(viewportHeightSync).toContain('document.addEventListener("visibilitychange", scheduleApplyHeight)');
